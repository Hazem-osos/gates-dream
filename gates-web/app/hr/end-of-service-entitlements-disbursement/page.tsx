...
import {
  endOfServiceEntitlementsDisbursementFormSchema,
  type EndOfServiceEntitlementsDisbursementFormInput,
} from '@/lib/validation/hr.schema';

const defaults: EndOfServiceEntitlementsDisbursementFormInput = {
  paymentMethod: 'صندوق',
  serialNumber: '',
...
export default function EndOfServiceEntitlementsDisbursementPage() {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<EndOfServiceEntitlementsDisbursementFormInput>({
    resolver: zodResolver(endOfServiceEntitlementsDisbursementFormSchema) as Resolver<EndOfServiceEntitlementsDisbursementFormInput>,
  ...
  const onSave: SubmitHandler<EndOfServiceEntitlementsDisbursementFormInput> = (values) => {
    console.info('[end-of-service-entitlements-disbursement]', values);
  };
  ...
  return (
    <HrPageChrome title="صرف مستحقات نهاية الخدمة">
            ...
            <div className="mb-8 space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4 lg:order-2">
                  ...
                  <div>
                    <label className="block text-sm text-[#094C6B] mb-2">نهاية الخدمة</label>
                    <div className="flex items-center gap-2">
                      ...
                      >
                        ...
                        <option value="إنهاء نهائي">إنهاء نهائي</option>
                        <option value="تسوية">تسوية</option>
                      </select>
                      <input
                        type="text"
                        className={`${flexInput} ${errors.amount ? 'border-red-400' : ''}`}
                        placeholder="رقم/مرجع نهاية الخدمة"
                        {...register('amount')}
                      />
                    </div>
                  </div>
