...
const defaults: AnnualLeaveEntitlementsDisbursementFormInput = {
  paymentMethod: 'صندوق',
  serialNumber: '',
  employee: '1212378971212',
  date: '26-11-2025',
  hijriDate: '26-11-2025',
  vacationId: '1212378971212',
  amount: '',
  notes: '',
  record: '0000012345',
};
...
export default function AnnualLeaveEntitlementsDisbursementPage() {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<AnnualLeaveEntitlementsDisbursementFormInput>({
    resolver: zodResolver(annualLeaveEntitlementsDisbursementFormSchema) as Resolver<AnnualLeaveEntitlementsDisbursementFormInput>,
  ...
  const onSave: SubmitHandler<AnnualLeaveEntitlementsDisbursementFormInput> = (values) => {
    console.info('[annual-leave-entitlements-disbursement]', values);
  };
            ...
            <div className="mb-8 space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4 lg:order-2">
                  ...
                  <div>
                    <label className="block text-sm text-[#094C6B] mb-2">الأجازة</label>
                    <div className="flex items-center gap-2">
                      <select
                        className={`px-3 py-2 border border-[#D6EAF3] bg-[#F6FBFD] rounded-lg text-[#094C6B] w-40 ${errors.vacationId ? 'border-red-400' : ''}`}
                        value={watch('vacationId')}
                        onChange={(e) => setValue('vacationId', e.target.value, { shouldValidate: true })}
                      >
                      ...
                      <input
                        type="text"
                        className={`${flexInput} ${errors.vacationId ? 'border-red-400' : ''}`}
                        placeholder="رقم الأجازة"
                        {...register('vacationId')}
                      />
                    ...
                    {errors.vacationId?.message ? <span className={errCls}>{String(errors.vacationId.message)}</span> : null}
                  </div>
