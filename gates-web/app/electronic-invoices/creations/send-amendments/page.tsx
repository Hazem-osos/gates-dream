...
export default function SendAmendmentsPage() {
  ...
  const { data: listResponse, isLoading: listLoading } = useApiQuery<ElectronicInvoiceListRow[]>(
    ['e-invoice-creation', 'amendment', filterData.clientCode, filterData.invoiceDateFrom, filterData.invoiceDateTo],
    ...
    <div className="p-4" style={{ direction: 'rtl' }}>
      ...
      <div className="mb-4">
        <div className="text-right">
          <h1 className="text-lg font-bold text-[#0E78AA] mb-1">إرسال تعدبلات الفواتير الإلكترونية  </h1>
        ...
        <InnerCard>
          <div className="p-4">
            ...
            <div className="overflow-x-auto rounded-2xl border border-[#E6F0F7] bg-white shadow-md min-h-[120px]">
              {!showInvoices ? (
                <EmptyState title="اضغط «عرض الفواتير» لتحميل فواتير التعديل المسودة من الخادم." />
              ) : listLoading ? (
                <TableSkeleton columns={12} rows={6} />
              ) : tableData.length === 0 ? (
                <EmptyState title="لا توجد فواتير تعديل مسودة." />
              ) : (
