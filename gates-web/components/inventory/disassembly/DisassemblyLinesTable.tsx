...
export function DisassemblyLinesTable({ lines, onChange, warehouseId, disabled }: Props) {
  ...
  const removeRow = (index: number) => {
    if (lines.length <= 1) {
      onChange([emptyDisassemblyComponentLine()]);
      return;
    }
  ...
  useClipboardTablePaste(wrapRef, {
    ...
    onPaste: (text) => {
      ...
      mapped.forEach((row, offset) => {
        ...
        while (next.length <= index) next.push(emptyDisassemblyComponentLine());
  ...
  return (
    ...
    >
      <UniversalDataGrid
        ...
        addLabel="إضافة مكون ناتج يدوياً (Enter)"
        disabled={disabled}
        renderCell={(index, columnId) => {
          const line = lines[index] ?? emptyDisassemblyComponentLine();
