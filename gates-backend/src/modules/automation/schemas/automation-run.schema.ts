import { z } from 'zod';
import { AUTOMATION_ACTION_RUN_STATUSES } from '../services/automation-action-run.types';
import { ACTION_RUN_LIST_DEFAULT, ACTION_RUN_LIST_MAX } from '../services/automation-action-run.service';

/**
 * User-facing (JWT + tenant session) run history query. companyId is NEVER
 * accepted here — it is always derived from the authenticated session,
 * mirroring every other `/api/v1/*` route in this app. This is distinct
 * from `internalAutomationActionRunListQuerySchema`, which is the n8n/S2S
 * surface and requires an explicit companyId because it has no session.
 */
const pageSchema = z
  .union([z.string(), z.number()])
  .optional()
  .transform((val) => {
    const n = typeof val === 'number' ? val : val ? parseInt(val, 10) : 1;
    return Number.isFinite(n) && n > 0 ? n : 1;
  });

const limitSchema = z
  .union([z.string(), z.number()])
  .optional()
  .transform((val) => {
    const n = typeof val === 'number' ? val : val ? parseInt(val, 10) : ACTION_RUN_LIST_DEFAULT;
    return Number.isFinite(n) && n > 0 ? Math.min(n, ACTION_RUN_LIST_MAX) : ACTION_RUN_LIST_DEFAULT;
  });

export const automationRunListQuerySchema = z.object({
  eventId: z.string().trim().min(1).optional(),
  eventType: z.string().trim().min(1).optional(),
  ruleId: z.string().uuid('ruleId must be a UUID').optional(),
  status: z.enum(AUTOMATION_ACTION_RUN_STATUSES).optional(),
  page: pageSchema,
  limit: limitSchema,
});

export const automationRunIdParamSchema = z.object({
  id: z.string().uuid('Run id must be a UUID'),
});

export type AutomationRunListQuery = z.infer<typeof automationRunListQuerySchema>;
