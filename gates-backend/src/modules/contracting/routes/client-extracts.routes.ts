...
import { clientExtractService } from '../services/client-extract.service';
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const data = await clientExtractService.createDraft(companyId, req.body);
      return void res.status(201).json({ status: 'success', data });
    } catch (e) {
      const status = e instanceof AppError ? e.statusCode : 500;
      return void res.status(status).json({
        status: 'error',
        message: e instanceof Error ? e.message : 'Create client extract failed',
      });
    }
  }
);
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      const data = await clientExtractService.list(
        companyId,
      ...
      return void res.status(status).json({
        status: 'error',
        message: e instanceof Error ? e.message : 'List client extracts failed',
      });
    }
  }
);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const data = await clientExtractService.post(ctx, req.params.id);
      return void res.json({ status: 'success', data });
    } catch (e) {
      const status = e instanceof AppError ? e.statusCode : 500;
      return void res.status(status).json({
        status: 'error',
        message: e instanceof Error ? e.message : 'Post client extract failed',
      });
    }
  }
);
