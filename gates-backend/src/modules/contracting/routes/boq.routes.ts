  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      return void res.status(status).json({
        status: 'error',
        message: e instanceof Error ? e.message : 'List BOQ failed',
      });
    }
  }
);
...
router.put(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const data = await projectBoqService.replaceAll(companyId, req.params.projectId, items);
      return void res.json({ status: 'success', data });
    } catch (e) {
      const status = e instanceof AppError ? e.statusCode : 500;
      return void res.status(status).json({
        status: 'error',
        message: e instanceof Error ? e.message : 'Save BOQ failed',
      });
    }
  }
);
